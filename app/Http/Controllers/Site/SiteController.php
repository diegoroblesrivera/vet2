...
class SiteController extends Controller
{
    ...
    {
        $param1 = "Dante";
        $mail = "dark.diegolas@gmail.com";
        $param3 = "Valor3";
        $param4 = "Dante2";
        $mail2 = "dark.diegolas@gmail.com";
        $param6 = "Valor32";

        Mail::to($mail)->send(new TestEmail($param1, $mail, $param3,$param4, $mail2, $param6));

        return "Correo electrónico nuevo enviado correctamente.".$mail ;
    }
        ...
        try {
            ...
            if ($schedule != null && $service != null) {
                ...
                foreach ($avaiableService as $key => $val) {
                    if ($bookingRepo->serviceIsAvaiable( $request->sch_employee_id, $date, $val['start_time'], $val['end_time']) > 0)
        ...
        DB::beginTransaction();
         try {
            ...
            if ($customer != null) {
                ...
                $fecha = Carbon::createFromFormat('d-m-Y', $request->nac)->format('Y-m-d');
                ...
                $savePet= [
                    ...
                    'sexo' => $request->sex,
                    //'otro_esp' => $request->otro_esp,
                    'created_by' => 1,
                    'raza' => $request->race,
                    'color' => $request->color,
                    'estado_repro' => $request->state,
                    'pet_birth_date' => $fecha 
                ...
                $userId =   User::create(
                    ...
                    CmnCustomer::where('id', $customerId)->update(['user_id' => $user_id]);
            ...
            foreach ($request->items as $key => $item) {
                ...
                $serviceList[] = [
                    ...
                    'remarks' => $request->motivo,
                    'created_by' => $customerId
                ];
                Mail::to($request->email)->send(new TestEmail($item->service_date, $serviceStartTime,$request->email, 
                $request->full_name,$request->pet_name, $request->motivo));
    ...
    // public function saveBooking(Request $request)
    // {  //dd($request);
    // //     $formattedDate = Carbon::createFromFormat('d-m-Y', $request->fecha)->format('Y-m-d');
    // // $saveCustomer['fecha'] = $formattedDate;
    ...
    public function saveBooking1(Request $request)
    {  //dd($request);
        Log::info($request->all());
    ...
    {      // dd($request); 227120245
        ...
        DB::beginTransaction();
         try {
            ...
            //     // if ($request->payment_type == PaymentType::UserBalance)
            //     //     throw new ErrorException(translate("You can't make payment by user balance without login try another one"));
            //     $customer = CmnCustomer::where('phone_no', $request->input('phone_no'))->orWhere('email', $request->input('email'))->select('id', 'phone_no')->first();

            // }
            // if ($customer != null) {
            //     $customerId = $customer->id;
            // } else {
                $fecha = Carbon::createFromFormat('d-m-Y', $request->input('nac'))->format('Y-m-d');
                ...
                Log::info('Justo antes de guardar el usuario');
                $cstRtrn = CmnCustomer::create($saveCustomer);
                $customerId = $cstRtrn->id;

                $savePet= [
                    'id_dueno' => $customerId ,
                    'nombre' => $request->input('pet_name'),
                    'especie' => $request->input('specie'),
                    'sexo' => $request->input('sex'),
                    // 'otro_esp' => $request->input('otro_esp'),
                    'created_by' => 1,
                    'raza' => $request->input('race'),
                    'color' => $request->input('color'),
                    'estado_repro' => $request->input('state'),
                    'pet_birth_date' => $fecha  
                ];
                $petRtn = CmnPet::create($savePet);
                $petId = $petRtn->id;
            ...
            $serviceTotalAmount = 0;
                ...
                $serviceTime = explode('-', $request->input('service_time'));
                $serviceStartTime = $serviceTime[0];
                $serviceEndTime = $serviceTime[1];
                ...
                $serviceStatus = ServiceStatus::Ensala;
                    // Assuming $request->input('service_date') contains the date in 'dd-mm-yyyy' format
                    $serviceDate = $request->input('service_date');

                    // Convert to Carbon instance and then to 'yyyy-mm-dd' format
                    $formattedDate = Carbon::createFromFormat('d-m-Y', $serviceDate)->format('Y-m-d');
                ...
                $serviceList[] = [
                    ...
                    'cmn_customer_id' => $customerId,
                    'cmn_pet_id' => $petId,
                    'sch_employee_id' => $request->input('sch_employee_id'),
                    'date' =>  $formattedDate,
                    'start_time' =>  $serviceStartTime,
                    'end_time' => $serviceEndTime,
                    'sch_service_id' => 2,
            ...
            //  Notification::send($user, new ServiceOrderNotification($bookingRepo->getServiceInvoice($serviceBookingInfo->id)));
            // }
            Mail::to($request->input('email'))->send(new TestEmail($formattedDate, $serviceStartTime, $request->input('sch_employee_id'),$request->input('full_name'), $request->input('pet_name'), $request->input('motivo')));
            // $this->enviarCorreo($formattedDate, $serviceStartTime, $request->input('sch_employee_id'),$request->input('full_name'), $request->input('pet_name'), $request->input('motivo'));
             // Aquí agregarías la lógica para crear las demás entidades, como mascotas, y programar la cita

             DB::commit(); // Confirma los cambios
            return redirect()->route('booking.complete')->with('success', 'Reserva guardada con éxito.');
         } catch (\Exception $e) {
             DB::rollBack(); // Revierte los cambios en caso de error
             Log::error("Error al guardar la reserva: " . $e->getMessage());
             return back()->withInput()->with('error', 'Error al guardar la reserva: ' . $e->getMessage());
         }
    }
    ...
    public function serviceDuePayment(Request $request)
    {
        try {
            ...
            if ($bookingInfo != null) {
                ...
                if ($bookingInfo->status != ServiceStatus::Cancel && $bookingInfo->status != ServiceStatus::Atendido && $dueAmount > 0) {
                    $paymentTypeForRtrn = 'paypal';
