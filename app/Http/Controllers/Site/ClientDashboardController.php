...
class ClientDashboardController extends Controller
{
    ...
    public function clientDashboard()
    {
        ...
        $rtrData=[
            'done'=>collect($customerBookingStatus)->where('status',ServiceStatus::Atendido)->sum('serviceCount'),
            'cancel'=>collect($customerBookingStatus)->where('status',ServiceStatus::Cancel)->sum('serviceCount'),
            'Confirmado'=>collect($customerBookingStatus)->where('status',ServiceStatus::Confirmado)->sum('serviceCount'),
            'others'=>collect($customerBookingStatus)->whereNotIn('status',[ServiceStatus::Cancel,ServiceStatus::Atendido])->sum('serviceCount'),
        ];
    ...
    public function availableToConfirmarBooking(Request $request)
    {
       try {  
            $repo = new BookingRepository();
            $rtrData = $repo->availableToConfirmarBooking($request->bookingId,3,auth()->user());
            return $this->apiResponse(['status' => '1', 'data' =>$rtrData], 200);         
