...
class SchServiceBookingController extends Controller
{
    ...
    public function saveBooking(Request $request)
    {
        ...
        try {
            ...
            foreach ($request->items as $key => $item) {
                ...
                $serviceLimitation = $bookingRepo->IsServiceLimitation($item->service_date, $request->cmn_customer_id, $item->sch_service_id, 1, 1);
            ...
            foreach ($serviceList as $service) {
                ...
                $service->status = ServiceStatus::Confirmado;
            }
    ...
    {
        $emailExists = CmnCustomer::where('email', $request->email)->exists();
    ...
    public function cambiarEstadoUltimaCita(Request $request, $idMascota)
{
    ...
    $ultimaCita = SchServiceBooking::where('cmn_pet_id', $idMascota)
        ->orderBy('date', 'desc')
        ->first();

    // Verificar si se encontró alguna cita para la mascota
    if ($ultimaCita) {
        // Cambiar el estado de la última cita a 6
        $ultimaCita->status = 6;
        $ultimaCita->save();

        // Redirigir a la ruta 'home'
        return redirect()->route('home')->with('success', 'Estado de la última cita actualizado correctamente');
    } else {
        // Si no se encontró ninguna cita para la mascota
        return redirect()->route('home')->with('error', 'No se encontraron citas para la mascota');
    }
}
}
