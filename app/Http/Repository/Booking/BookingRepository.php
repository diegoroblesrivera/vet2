...
class BookingRepository
{
        ...
        if ($this->IsServiceAvailableToCancel($bookingId) > 0) {
            ...
            if ($emailNotify == 1 && $user != null && UtilityRepository::isEmailConfigured()) {
                ...
                $serviceMessage = [
                    ...
                    'message_body' => 'Su servicoo ha sido  ' . UtilityRepository::serviceStatus($updateData->status) . '.',
                    'booking_info' => ' Cita No#' .  $updateData->id . ', Fecha de servicio# ' . $serviceDate->format('D, M d, Y') . ' at ' . $updateData->start_time . ' to ' .  $updateData->end_time,
                    'message_footer' => 'Gracias por elegirnos.',
    ...
    public function availableToConfirmarBooking($bookingId, $emailNotify, $user)
    {
        if (($bookingId) > 0) {
            $updateData = SchServiceBooking::where('id', $bookingId)->first();
            $updateData->status = ServiceStatus::Confirmado;
            $updateData->updated_by = Auth::id();
            $updateStatus = $updateData->update();
            ...
            if ($emailNotify == 1 && $user != null && UtilityRepository::isEmailConfigured()) {
                ...
                $serviceMessage = [
                    'user_name' => $user->name,
                    'message_subject' => 'Booking ' . UtilityRepository::serviceStatus($updateData->status) . ' Notification',
                    'message_body' => 'Su servicio ha sido  ' . UtilityRepository::serviceStatus($updateData->status) . '.',
                    'booking_info' => ' Cita No#' .  $updateData->id . ', Fecha de Servicio# ' . $serviceDate->format('D, M d, Y') . ' at ' . $updateData->start_time . ' to ' .  $updateData->end_time,
                    'message_footer' => 'Gracias por elegirnos.',
                    'action_url' => url('/client-dashboard')
                ];
    ...
    public function ChangeBookingStatus($bookingId, $status, $emailNotify = 0)
    {
        ...
        if ($emailNotify == 1) {
            ...
            if ($customer != null && $customer->user_id != null && UtilityRepository::isEmailConfigured()) {
                ...
                // service feedback
                if ($status == ServiceStatus::Atendido) {
    ...
    public function serviceIsAvaiable( $employeeId, $date, $serviceStartTime, $serviceEndTime)
    {
        $serviceStatus = [ServiceStatus::Confirmado,ServiceStatus::RecordatorioEnviado, ServiceStatus::Atendido,
         ServiceStatus::RecordatorioEnviado, ServiceStatus::Ensala];
        ...
        if ($service != null) {
            ...
            if ($checkServiceLimit == 1) {
                ...
                if ($service->appoinntment_limit_type == AppointmentLimitType::Unlimited) {
                    ...
                    $serviceSts = [ServiceStatus::Atendido, ServiceStatus::Confirmado, ServiceStatus::RecordatorioEnviado, ServiceStatus::Ensala];
            ...
            // if ($checkBookingLimit == 1) {
            //     //check service booking is available
            //     $nowDate = (new Carbon())->subMinutes(5);
            //     $selectedServiceDate = new Carbon($serviceDate . ' ' . $nowDate->format('H:i'));
            //     $minimumTimeRequiredToBookingInMinute = DateTimeRepository::TotalMinuteFromTime($service->minimum_time_required_to_booking_in_time);
            
            //     // Variables de depuración
            //     $debugInfo = [
            //         'now' => $nowDate->toDateTimeString(),
            //         'selectedServiceDate' => $selectedServiceDate->toDateTimeString(),
            //         'minimumTimeRequiredToBookingInMinute' => $minimumTimeRequiredToBookingInMinute
            //     ];
            
            //     if ($selectedServiceDate->isToday()) {
            //         $serviceAvaiableFromBooking = $nowDate->copy()->addMinutes($minimumTimeRequiredToBookingInMinute);
            //         $debugInfo['serviceAvaiableFromBookingToday'] = $serviceAvaiableFromBooking->toDateTimeString();
            //     } else {
            //         $serviceAvaiableFromBooking = $nowDate->copy()->addDays($service->minimum_time_required_to_booking_in_days)->addMinutes($minimumTimeRequiredToBookingInMinute);
            //         $debugInfo['serviceAvaiableFromBookingFuture'] = $serviceAvaiableFromBooking->toDateTimeString();
            //     }
            
            //     if ($selectedServiceDate >= $serviceAvaiableFromBooking) {
            //         $allowBookingLimit = 1;
            //     } else {
            //         $debugInfo['message'] = "No se permite reservar: La fecha seleccionada es demasiado pronto.";
            //         return ['allow' => 0, 'debug' => $debugInfo, 'message' => "Para Agendar , necesitas un minimo de  " . $service->minimum_time_required_to_booking_in_days .
            //          ' dias & ' . $minimumTimeRequiredToBookingInMinute . ' y minutos.'.$debugInfo['selectedServiceDate'], 'for' => 'ServiceBookingLimit'];
            //     }
            // }
            
            // Asegúrate de quitar la información de depuración en la versión final de la API
    ...
    {
        $bookedService = SchServiceBooking::where('id', $serviceBookingId)->whereNotIn('status', [ServiceStatus::Atendido])
