                ...
                dTable = $('#tableElement').DataTable({
                    ...
                    columns: [
                        ...
                        },
                        {
                            ...
                            width: 150,
                            render: function (data, type, row) {
                                var customerPhone = row['customer_phone_no'];
                                var appointmentDetails = 'Hola, te recordamos tu cita para el día ' + moment(row['date']).format('MMM DD, YYYY') +
                                                         ' de ' + moment(row['start_time'], 'HH:mm:ss').format('LT') +
                                                         ' a ' + moment(row['end_time'], 'HH:mm:ss').format('LT');
                        
                                var whatsappLink = 'https://wa.me/' + customerPhone + '?text=' + encodeURIComponent(appointmentDetails);
                                
                                var actionButton = '<button class="btn btn-sm btn-primary dt-button-action"><i class="fas fa-location-arrow"></i> Action</button>';
                                var reminderButton = '<a href="' + whatsappLink + '" target="_blank" class="btn btn-sm btn-success dt-button-action"><i class="fab fa-whatsapp"></i> Enviar Recordatorio</a>';
                                
                                return actionButton + ' ' + reminderButton;
                            }
                        }
                        ,
                    ],
